...
if (isset($_GET['q'])) {
    $datos = array();
    $nombre = $_GET['q'];
    $estudiante = mysqli_query($conexion, "SELECT * FROM estudiante WHERE nombre LIKE '%$nombre%' AND estado = 1");
    ...
    $insertar = mysqli_query($conexion, "INSERT INTO ventas(id_estudiante, total, id_usuario) VALUES ($id_estudiante, '$total', $id_user)");
    if ($insertar) {
        ...
        if ($insertarDet) {
            ...
            $msg = array('id_estudiante' => $id_estudiante, 'id_venta' => $ultimoId);
        } 
    }else{
